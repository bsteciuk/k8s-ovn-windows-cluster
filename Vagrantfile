...
#        windows.vm.box = "StefanScherer/windows_2016_docker"
        windows.vm.box = "marcinbojko/w1709-trial"
        #windows.vm.box = "windoes_server_2016_core"
        #windows.vm.box = "symbols/windows_2016_core"
    ...
    config.vm.define "build" do |linux|
        linux.vm.box = 'bento/ubuntu-16.04'
        linux.vm.hostname = 'build'
        linux.vm.network 'private_network', ip: '172.20.2.20', auto_config: true
        linux.vm.provider :virtualbox do |vb|
            vb.name = 'build'
            vb.memory = 4096
            vb.customize [
        ...
        linux.vm.provision "shell", path: "init-build.sh", privileged: false
        #linux.vm.provision "shell", inline: $hosts_script
        #linux.vm.provision "shell", path: "init.sh", privileged: true
        #linux.vm.provision :reload
        #linux.vm.provision "shell", path: "join.sh", privileged: true
